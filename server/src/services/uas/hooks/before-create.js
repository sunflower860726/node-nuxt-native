  ...
  return function(hook) {
    ...
    if (!hook.data.date) {
      errorMessages.push({
        ...
        message: 'UAS result must have a date'
      });
    }

    // Validations for existing data
    const UAS = hook.app.service('api/uas');
    return UAS.find({
      query: {
      ...
      if (uasResult && uasResult.length > 0) {
        errorMessages.push({
          path: 'date',
          value: hook.data.date,
          message: 'UAS result already submitted for this date'
        });
      }
