  ...
  return function(hook) {
    ...
    if (!hook.data.date) {
      errorMessages.push({
        ...
        message: 'Control test result must have a date'
      });
    }

    // Validations for existing data
    const ControlTest = hook.app.service('api/control-test');
    return ControlTest.find({
      query: {
      ...
      if (ctResult && ctResult.length > 0) {
        errorMessages.push({
          path: 'date',
          value: hook.data.date,
          message: 'Control test result already submitted for this date'
        });
      }
