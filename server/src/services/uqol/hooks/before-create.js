  ...
  return function(hook) {
    ...
    if (!hook.data.date) {
      errorMessages.push({
        ...
        message: 'UQOL result must have a date'
      });
    }

    // Validations for existing data
    const UQOL = hook.app.service('api/uqol');
    return UQOL.find({
      query: {
      ...
      if (uqolResult && uqolResult.length > 0) {
        errorMessages.push({
          path: 'date',
          value: hook.data.date,
          message: 'UQOL result already submitted for this date'
        });
      }
